...
class CharSkillSkeloSkillDeficit(PyMCModel):
    model_name = "char_skill_skelo_skill_deficit"
        ...
        with pm.Model() as model:
            ...
            win_chance_logit = pm.Deterministic(
                ...
                * mu[self.validation_input["mupT"] - 1]
                + elo_logit_scale * self.validation_input["skelo_logitT"],
            )
    ...
    def predict(self, games):
        ...
        return expit(
            ...
            - skill2
            + (non_mirror * matchup)
            + (elo_logit_scale * logit(games.elo_estimate))
        )
