...
parameters {
    ...
    vector[NP] char_skill_std[NC]; // Player skill at character
    vector[NC] char_skill_variance; // Per-character skill variance
...
transformed parameters {
    ...
    vector[NP] char_skill[NC];

    for (p in 1:NP) {
        for (c in 1:NC)
        char_skill[c, p] = char_skill_std[c, p] .* char_skill_variance[c];
    }
