...
class YomiRender:
    def __init__(self, data_name, model, warmup=1000, min_samples=1000):
        ...
        self.base_folder = f"images/{data_name}/{self.model.model_name}-{self.model.model_hash[:6]}"
    ...
    def player_char_skill(self):
        ...
        for sample in player_char_skill['sample'].unique():
            player_char_skill.loc[player_char_skill['sample'] == sample, 'char_skill'] -= sample_max_skill[sample]

        player_char_skill['win_chance'] = pandas.to_numeric((player_char_skill['char_skill'].rpow(math.e)) / (1 + player_char_skill['char_skill'].rpow(math.e)))
    ...
    def render_char_skill(self, player):
        ...
        skill_summaries = char_skill.groupby('character').char_skill.agg(['median', 'std'])
        ...
        char_skill['skill_median'] = char_skill.character.apply(lambda c: skill_summaries['median'][c]).astype(float) + 5
        char_skill['skill_std'] = char_skill.character.apply(lambda c: skill_summaries['std'][c]).astype(float)
        char_skill_chart = (
            ggplot(char_skill, aes(x='character', y='char_skill + 5',  fill='skill_median', alpha='skill_std',))
            + geom_violin(
                scale='width',
            )
        ...
        char_skill_chart.save(f'{self.base_folder}/{player}-char-skill.png', verbose=False)

    def render_char_win_chance(self, player):
        skill = self.player_char_skill
        char_skill = skill[skill.player == player]
        skill_summaries = char_skill.groupby('character').win_chance.agg(['median', 'std'])
        ...
        char_skill['skill_std'] = char_skill.character.apply(lambda c: skill_summaries['std'][c]).astype(float)
        char_skill_chart = (
            ggplot(char_skill, aes(x='character', y='win_chance',  fill='skill_median', alpha='skill_std',))
            + geom_violin(
                scale='width',
            )
            + scale_fill_distiller(type='div', palette='Spectral', labels=lambda l: ["%d%%" % (v * 100) for v in l], limits=(0, 0.5))
            + scale_alpha(range=(1, 0.2))
            + theme(
                figure_size=(char_skill.character.nunique()*.2, 2),
                axis_text_x=element_text(rotation=90),
            )
            + scale_y_continuous(labels=lambda l: ["%d%%" % (v * 100) for v in l], limits=(0, 0.5))
            + labs(title=player)
        )
        char_skill_chart.save(f'{self.base_folder}/{player}-char-win-chance.png', verbose=False)
    ...
    def render_matchup_chart(self):
        median_rates = pandas.to_numeric(self.matchups.groupby(['c1', 'c2']).win_rate.median().rename('median_win_rate'))
    ...
    def balanced_matchups(self):
        ...
        most_balanced = abs_unbalance.reset_index().sort_values(['c1', 'abs_unbalance']).groupby('c1').head(6)
        ...
        for character in Character:
            counterpicks = " ".join(most_balanced[most_balanced.c1 == character].c2.apply(lambda c: f":{c}:"))
    ...
    def render_matchup_comparator(self):
        ...
        matchups = summary[[col for col in summary.columns if col.startswith('mu[')]].rename(
            columns={'mu[{}]'.format(ix): "{.value}-{.value}".format(c1, c2) for ((c1, c2), ix) in mu_index.items()}
        ...
        player_char_skill = summary[[col for col in summary.columns if col.startswith('char_skill[')]].transpose().reset_index()
        player_char_skill['player'] = player_char_skill['index'].apply(
            lambda x: reverse_player_index[int(x[11:-1].split(',')[1])]
        ).astype(self.player_category)
