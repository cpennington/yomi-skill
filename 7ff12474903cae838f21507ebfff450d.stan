...
generated quantities{
    ...
    for (n in 1:NVG)
        log_lik_v_win[n] += bernoulli_logit_lpdf(v_win[n] | muv[v_mup[n]] .* (skill[v_pt1[n]] - skill[v_pt2[n]]) + mu[v_mup[n]]);
    for (n in 1:NVMG)
        log_lik_v_m_win[n] += bernoulli_logit_lpdf(v_m_win[n] | muv[v_m_mup[n]] .* (skill[v_m_pt1[n]] - skill[v_m_pt2[n]]) + mu[v_m_mup[n]]);
}
