...
data {
    ...
    int<lower=0, upper=1> v_win[NVG]; // Did player 1 win game
    int<lower=1, upper=NPT> v_pt1[NVG]; // Player/tournament 1 in game
    int<lower=1, upper=NPT> v_pt2[NVG]; // Player/tournament 2 in game
    int<lower=1, upper=NM> v_mup[NVG]; // Matchup in game
    
    int<lower=0, upper=1> v_m_win[NVMG]; // Did player 1 win game
    int<lower=1, upper=NPT> v_m_pt1[NVMG]; // Player/tournament 1 in mirror-match
    int<lower=1, upper=NPT> v_m_pt2[NVMG]; // Player/tournament 2 in mirror-match
    int<lower=1, upper=NM> v_m_mup[NVMG]; // Matchup in game
...
generated quantities{
    ...
    log_lik_v_win = rep_vector(0, NVG);
    log_lik_v_m_win = rep_vector(0, NVMG);
    
    for (n in 1:NVG)
        log_lik_v_win[n] += bernoulli_logit_lpmf(v_win[n] | muv[v_mup[n]] .* (skill[v_pt1[n]] - skill[v_pt2[n]]) + mu[v_mup[n]]);
    for (n in 1:NVMG)
        log_lik_v_m_win[n] += bernoulli_logit_lpmf(v_m_win[n] | muv[v_m_mup[n]] .* (skill[v_m_pt1[n]] - skill[v_m_pt2[n]]) + mu[v_m_mup[n]]);
}
