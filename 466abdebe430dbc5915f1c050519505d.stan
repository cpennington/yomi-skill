...
generated quantities{
    ...
    log_lik_v_win += bernoulli_lpdf(inv_logit(muv[v_mup] .* (skill[v_pt1] - skill[v_pt2]) + mu[v_mup]));
    log_lik_v_m_win += bernoulli_lpdf(inv_logit(muv[v_m_mup] .* (skill[v_m_pt1] - skill[v_m_pt2]) + mu[v_m_mup]));
}
